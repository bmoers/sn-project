...
const maxBuffer = 2 * 1024 * 1024; // about 1MB UTF-8
...
Git.prototype.clone = async function (target) {
    var self = this;
    
    // ensure all properties are set
    await _auth.call(self, true);

    return _execAsync.call(self, {
        quiet: true,
        args: `clone -n ${self.configuration.remoteUrl} ${target}`
    });
};
    ...
    return Promise.try(function () {
        ...
        return Promise.try(() => {
            ...
            return commandExists('ssh-keyscan').then(function () {
                console.log(`adding key file to known_hosts with command: '${cmd}'`);
                return new Promise(function (resolve, reject) {
                    //console.log(cmd);
...
var _auth = async function (global = false){
    var self = this;
    ...
    if (self.configuration.remoteUrl) {
        ...
        // ensure the credentials are correct set
        ...
        await self.config(`${globalFlag} credential.helper`, `store --file ${gitCredentialsFile}`);
...
var _init = async function (appendCommitMessage) {
    var self = this;
    
    await fs.ensureDir(self.configuration.dir);

    const initialized = await _gitInitialized.call(self);

    if (initialized) {
        console.log('git already initialized');
        return;
        /*
        const remoteAdded = await _addRemote.call(self);
        if(remoteAdded){
            await _rebaseAllRemoteBranches.call(self);       
        }
        return;
        */
    }
    ...
    if (self.configuration.user.name)
        await self.config('user.name', self.configuration.user.name);

    if (self.configuration.user.email)
        await self.config('user.email', self.configuration.user.email);

    // ensure the auth params are all set
    await _auth.call(self, false);

    if (self.configuration.remoteUrl) {
        // now add remote 
        await _addRemote.call(self);

        await _fetch.call(self);

        const exists = await _branchExists.call(self, 'master');
        if (exists){
            await _execAsync.call(self, {
                quiet: self.configuration.quiet,
    ...
    const gitIgnoreTouched = await _createGitIgnoreFile.call(self, true);
    if (gitIgnoreTouched) {
    ...
    if (self.configuration.remoteUrl) {
        ...
        await _pull.call(self);
    }

};

var _addRemote = async function () {
    var self = this;

    if (!self.configuration.remoteUrl)
        return false;

    const remoteName = await _execAsync.call(self, {
        args: 'remote',
        quiet: self.configuration.quiet
    });

    const currentName = remoteName.replace(/(\r\n|\n|\r)/gm, '');
...
const _rebaseAllRemoteBranches = async function () {
    var self = this;
    ...
    const localBranchNamesString = await _execAsync.call(self, {
        quiet: self.configuration.quiet,
        args: 'branch -l'
    });
    const localBranchNames = localBranchNamesString.split(/[\n\r]+/).filter((row) => (row && row.length)).map((row) => {
        return row.replace(/^\*?\s*/gm, '').trim();
    });
    console.log('localBranchNames:', localBranchNames);

    const remoteBranchNamesString = await _execAsync.call(self, {
        quiet: self.configuration.quiet,
        args: 'branch -r'
    });
    const remoteBranchNames = remoteBranchNamesString.split(/[\n\r]+/).filter((row) => (row && row.length)).map((row) => {
        return row.trim();
    });
    console.log('remoteBranchNames:', remoteBranchNames);

    const checkoutBranchNames = remoteBranchNames.filter((row) => !localBranchNames.includes(row.replace('origin/', '')));
    console.log('checkoutBranchNames:', checkoutBranchNames);

    return Promise.each(checkoutBranchNames, async function (checkoutBranchName) {
        await _execAsync.call(self, {
            quiet: self.configuration.quiet,
            args: `checkout ${checkoutBranchName.replace('origin/', '')}` // --track
        });
        await _execAsync.call(self, {
            quiet: self.configuration.quiet,
            args: 'fetch origin'
        ...
        await _execAsync.call(self, {
            quiet: self.configuration.quiet,
            args: `branch --set-upstream-to=${checkoutBranchName} ${checkoutBranchName.replace('origin/', '')}`
        });
        await _execAsync.call(self, {
            quiet: self.configuration.quiet,
            args: 'rebase'
        ...
        await _execAsync.call(self, {
            quiet: self.configuration.quiet,
            args: `pull origin ${checkoutBranchName.replace('origin/', '')}`
        });
    });
};


// git branch --set-upstream-to=origin/cicd-test-application-2-b-@d6892200db765490fcf41780399619d8 cicd-test-application-2-b-@d6892200db765490fcf41780399619d8
const _upstreamAllBranches = async function () {
    var self = this;
    ...
    return Promise.each(checkoutBranchNames, function (checkoutBranchName) {
        return _execAsync.call(self, {
            quiet: self.configuration.quiet,
            args: `checkout --track ${checkoutBranchName}`
        }).catch((e) => {
...
var _switchToBranch = async function () {
    ...
    const exists = await _branchExists.call(self, branchName);
    if (!exists)
        await _createBranch.call(self, branchName);
...
var _createBranch = async function () {
    var self = this;
    var branchName = _sanitizeBranchName.apply(self, arguments);
    console.log('Create branch: ', branchName);

    const exist = await _branchExists.call(self, branchName);
    if (exist) {
    ...
    return Promise.try(function () {
        if (self.configuration.remoteUrl) {
            ...
            return _branchConfig.call(self, branchName).then((branchConfig) => {
                ...
                const branch = (branchName !== undefined) ? `origin ${branchName}` : '--all';
                const args = `fetch ${branch}`;
...
var _pull = function (branchName = 'master') {
    var self = this;
    return Promise.try(function () {
        if (self.configuration.remoteUrl) {
            ...
            return _branchConfig.call(self, branchName).then((branchConfig) => {
                ...
                const args = `pull origin ${branchName}`; //  --rebase=false
                console.log(`GIT - PULL: ${args}`);
...
var _push = async function (branchName, force) {
    var self = this;
    /*
        add https support
    ...
    */
    ...
    const branch = (branchName !== undefined) ? `origin ${branchName}` : '--all';
    const args = `push ${(force === true) ? '--force' : ''} --set-upstream ${branch}`;

    console.log(`GIT - PUSH: ${args}`);
