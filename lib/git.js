...
var _createGitIgnoreFile = function (force) {
    ...
    return fs.stat(gitIgnoreFile).then(function () {
        if (!force) {
...
var _addToKnownHosts = function (remoteUrl) {
    ...
    if (!match || (match && (!match.groups.host || !match.groups.project))) {
    ...
    const os = require('os');
    const home = (os.platform() === 'win32') ? `%userprofile%` : `~`;
    ...
    return Promise.try(function () {
        ...
        var cmd = (port) ? `ssh-keyscan -p ${port} ${host} >> ${home}/.ssh/known_hosts` : `ssh-keyscan ${host} >> ${home}/.ssh/known_hosts`;
        ...
        }).then((knownHostRecordExitst) => {
            if (knownHostRecordExitst) {
            ...
            return commandExists('ssh-keyscan').then(function () {
                ...
                */
                return commandExists('plink').then(function () {
                    ...
                    console.log("adding key via plink", cmd);
                    return new Promise(function (resolve, reject) {
                        //console.log(cmd);
                        exec(cmd, { maxBuffer: 200 * 1024 }, function (err, stdout, stderr) {
...
var _init = function (appendCommitMessage) {
    ...
    return fs.stat(self.configuration.dir).catch({
        code: 'ENOENT'
    }, function () {
            ...
            return Promise.try(function () {
                if (self.configuration.remoteUrl) { // && notEmptyLocalDir === false
                    ...
                    return _addToKnownHosts.call(self, self.configuration.remoteUrl).then(function () {
                        ...
                        */
                        let url = self.configuration.remoteUrl;
                        if (!self.configuration.user.store &&
                            self.configuration.user.name && self.configuration.user.password) {
                            // add the creds to the url
                            url = url.replace(/^https:\/\//, `https://${self.configuration.user.name}:${self.configuration.user.password}@`)
                        }
                        console.log("clone from remote", url, self.configuration.dir);

                        return _execAsync.call(self, {
                            quiet: self.configuration.quiet,
                            args: ['clone', url, self.configuration.dir].join(' ')
                        });
                    });
                } else {
                ...
                // add the creds to the local store
                if (self.configuration.user.store && self.configuration.user.name && self.configuration.user.password)
                    return Promise.try(() => {

                        return self.config('credential.helper', `'store'`);
                    }).then(() => {
                        return new Promise((resolve, reject) => {
                            var cp = require('child_process');
                            var child = cp.spawn('git', ['credential', 'approve'], { cwd: self.configuration.dir, detached: false });
                            var log = '';
                            ...
                            child.stdin.write(`url=${self.configuration.remoteUrl}`);
                            child.stdin.write(`username=${self.configuration.user.name}`);
                            child.stdin.write(`password=${self.configuration.user.password}`);
                            child.stdin.write('');
                            //child.stdin.write('');
                            child.stdin.end();
...
var _commit = function (options) {
    ...
    return Promise.try(function () {
        ...
        return tmp.fileAsync({ keep: false }).then(function (tempFile) {
            ...
            */
            return fs.writeFile(tempFile, opt.messages.join('\n'), {
                encoding: 'utf8'
            }).then(function () {
                return tempFile;
            });
