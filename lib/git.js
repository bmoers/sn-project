...
var _init = function (appendCommitMessage) {
    ...
    return fs.ensureDir(self.configuration.dir).then(() => {
        ...
        return Promise.try(function () {
            ...
            return Promise.try(function () {
                return _addToKnownHosts.call(self, self.configuration.remoteUrl);

            }).then(async function () {

                // add the credentials to the local store
                if (!self.configuration.user.store)
                    return console.info('credential store is disabled');

                if (!self.configuration.user.name || !self.configuration.user.password)
                    return console.warn('CICD_GIT_USE_CRED_STORE is enabled but CICD_GIT_USER_NAME or CICD_GIT_USER_PASSWORD not set.')


                const m = (/^(https?\:\/\/)([^\/?]*)/).exec(self.configuration.remoteUrl)
                if (!m)
                    return console.error(`URL not suitable for credential store: ${self.configuration.remoteUrl}`);

                const url = `${m[1]}${self.configuration.user.name}:${self.configuration.user.password}@${m[2]}`;
                const file = path.resolve(`./.git-credentials`);

                await fs.ensureFile(file);

                let body = await fs.readFile(file, { encoding: 'utf8' });
                if (!body.includes(url)){
                    await fs.appendFile(file, `\n${url}`, { encoding: 'utf8' });
                }

                console.log(`set credential.helper to 'store --file ${file}'`);

                return self.config('credential.helper', `store --file ${file}`);
                ...
                */
                if(self.configuration.user.store)
                    return console.log('credential store used for authentication');
                
                if (!self.configuration.user.name || !self.configuration.user.password)
                    return console.log(`No Git credentials specified. Using local certificate.`);
