 ...
 * @param {String} rangeStart pattern as described in https://git-scm.com/docs/gitrevisions
 * @param {String} [rangeEnd=HEAD] pattern as described in https://git-scm.com/docs/gitrevisions
 * @returns {Array} the files found in the given directory
 */
Git.prototype.listFiles = function (directory, rangeStart, rangeEnd) {
    var self = this;
    return _listFiles.call(self, directory, rangeStart, rangeEnd);
};
...
var _listFiles = function (directory, rangeStart, rangeEnd = 'master') { // default list all files in master branch only, set to HEAD to show latest changes
    var self = this;

    return _execAsync.call(self, {
        quiet: self.configuration.quiet,
        args: `log --name-status --format="" ${rangeStart ? `${rangeStart}...` : ''}${rangeEnd} -- ${directory}`
    }).then((log) => {
